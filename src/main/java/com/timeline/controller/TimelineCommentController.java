...
public class TimelineCommentController {
    ...
    public ResponseEntity update(@PathVariable Long id, @RequestBody TimelineCommentRequestDto timelineCommentRequestDto) {
        return ResponseEntity.ok(timelineCommentService.update(id, timelineCommentRequestDto));
    }

    /* 타임라인 댓글 삭제 */
    @DeleteMapping(path = "/comment/{id}")
    public boolean delete(@PathVariable Long id) {
        return timelineCommentService.delete(id);
    }
